    ...
    describe('when logged in', () => {
        ...
        describe('and a blog exists', () => {
            ...
            test('a blog can be liked', async ({ page }) => {
                await page.pause()
                const otherBlogText = await page.getByText(
                    'McLovin2 Seth Rogen'
                )
                ...
                await otherBlogElement
                    ...
                    .click()
                await expect(
                    otherBlogElement.getByText('likes 0')
                ).toBeVisible()
                await otherBlogElement
                    .getByRole('button', { name: 'like' })
                    .click()
                await expect(
                    otherBlogElement.getByText('likes 1')
                ).toBeVisible()
            })
            ...
            test('user cannot delete a blog created by another user', async ({
                page,
                request,
            }) => {
                await createBlog(
            ...
            test('blogs are ordered by likes', async ({ page }) => {
                ...
                const likes = await page
                    .getByTestId('number-of-likes')
                    .allTextContents()
                await expect(likes).toEqual(['9', '5', '0'])
            })
        })
    })
})
